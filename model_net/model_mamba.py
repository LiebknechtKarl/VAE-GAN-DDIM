...
@dataclass
class ModelArgs:
    input_channel : int = 62
    output_channel : int = 25
    d_model: int = 128
    n_layer: int = 4
...
class Mamba(nn.Module):
    def __init__(self, args: ModelArgs):
        ...
        self.input_linear = nn.Linear(args.input_channel, args.d_model, bias=False)
        self.layers = nn.ModuleList([ResidualBlock(args) for _ in range(args.n_layer)])
        self.norm_f = RMSNorm(args.d_model)

        self.lm_head = nn.Linear(args.d_model, args.vocab_size, bias=False)
        self.output_linear = nn.Linear(args.d_model, args.output_channel, bias=False)
...
if __name__ == "__main__":
    ...
    # # Simple test to verify model can do a forward pass
    # pretrained_model_name = 'state-spaces/mamba-370m'
    # model = Mamba.from_pretrained(pretrained_model_name)
    # input_ids = torch.tensor([[46, 31834, 310, 253]])  # Example input
    # logits = model(input_ids)
    # print(logits.shape)  # Should be (1, 4, vocab_size)
    ...
    # x = torch.randint(low=0, high=10, size=(2, 64, 128), dtype=torch.int32)
    # x = torch.tensor([[46,31834,310,253]])
    x = torch.tensor([[46,31834,310,253, 253]])
    x1 = torch.randn(2, 64, 62)

    # torch.Size([16, 64, 1]) torch.Size([16, 64, 61]) torch.Size([16, 64, 25])

    # y = model(x)
    y = model(x1)

    print(y.shape)  # Should be (2, 64, 25)