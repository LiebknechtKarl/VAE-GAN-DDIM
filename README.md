...
这个存储库已经过时了，因为我很遗憾没有时间再维护它了。如果您想作为合作者继续开发它，请发送电子邮件至eriklindernoren@gmail.com.**

pyTorch GAN
研究论文中展示的生成对抗网络变体的PyTorch实现集合。模型架构并不总是反映论文中提出的架构，但我选择专注于涵盖核心思想，而不是正确地进行每一层配置。我们非常欢迎GANs为实施GAN所做的贡献和提出的建议。
...
####作者
奥古斯都·奥德纳、克里斯托弗·奥拉、乔纳森·施莱斯

####摘要
合成高分辨率的真实感图像一直是机器学习领域的一个长期挑战。本文介绍了用于图像合成的生成对抗网络（GAN）改进训练的新方法。我们构建了一种采用标签条件的GAN变体，其结果是128x128分辨率的图像样本表现出全局一致性。我们扩展了之前的图像质量评估工作，为评估类条件图像合成模型中样本的可辨性和多样性提供了两种新的分析方法。这些分析表明，高分辨率样本提供了低分辨率样本中不存在的类别信息。在1000个ImageNet类中，128x128样本的可分辨性是人工调整大小的32x32样本的两倍多。此外，84.7%的类具有与真实ImageNet数据相当的多样性样本。
...
在这篇论文中，我们提出了“对抗性自动编码器”（AAE），这是一种概率自动编码器，它使用最近提出的生成对抗网络（GAN）通过将自动编码器的隐藏码向量的聚合后验与任意先验分布进行匹配来执行变分推理。将聚合后的后验与先验相匹配，可以确保从先验空间的任何部分生成有意义的样本。因此，对抗性自动编码器的解码器学习了一个深度生成模型，该模型在数据分发之前映射了所施加的内容。我们展示了对抗性自动编码器如何用于半监督分类、图像风格和内容的解纠缠、无监督聚类、降维和数据可视化等应用。我们在MNIST、街景门牌号和多伦多人脸数据集上进行了实验，结果表明，对抗性自动编码器在生成建模和半监督分类任务中取得了有竞争力的结果。
